...
class Server {
    ...
    rutas() {
        ...
        this.app.post('/login',
            ...
            async function (req, res) {
                const erroresVal = validationResult(req);
                //Comprueba si ha habido errores en los check
                if (!erroresVal.isEmpty()) {
                    return res.status(400).json({ msg: erroresVal.array() });
                }
        ...
        this.app.get('/destino/:id', async function (req, res) {
            const id = req.params.id;
            let destino = await Destino.findById(id);
            res.json(
                destino

            )
        })
        this.app.get('/destinos',validarJWT, async function (req, res) {
